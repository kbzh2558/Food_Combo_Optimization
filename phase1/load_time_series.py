...
def process_single_date_pandas(
        ...
        try:
            ...
            try:
                ddf = dd.read_parquet(
                    file_path,
                    engine='pyarrow',
                    columns=['business_date','product_desc','quantity_sold','sales_amount'],
                    filters=[("product_desc", "in", unique_products)]
                )
    ...
    # wanted col
    transaction_item = transaction_item[['business_date','product_desc','site_number','quantity_sold','sales_amount']].copy()
    # only quebec
    transaction_item = transaction_item[transaction_item['site_number'].isin(site_list)]

    print('------read items done------')
    del item_dfs
    
    # Filter out products
    transaction_item = transaction_item[transaction_item['product_desc'].isin(unique_products)]
    ...
    grouped_df = transaction_item.groupby(['business_date', 'product_desc']).agg({
        'quantity_sold': 'sum',
        'price': 'mean'
    }).reset_index()
...
dump(data, os.path.join(base_dir, "data_processed", "product_time_series.pkl"))

print("✅ Dictionary saved successfully!")

# concat
stacked_df = pd.concat(data.values(), keys=data.keys()).reset_index(drop=True)
stacked_df.to_csv(os.path.join(base_dir, "data_processed", "product_time_series.csv"), index=False)
